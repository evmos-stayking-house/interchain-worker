...
export class StaykingLiquidationService {
  ...
  async killPositions(killablePositions: KillablePosition[], vaultContract: Contract) {
    ...
    for (const { positionId } of killablePositions) {
      ...
      try {
        ...
        console.log(`Your cron function for liquidation in ${this.stayKingContract.name_} for position ${positionId} works`);
      } catch (error) {
  ...
  async aggregateKillablePositions(posLen: number, vaultContract: Contract): Promise<KillablePosition[]> {
    const tokenAddress = await vaultContract.token();
    const checkPositions = Array.from({ length: posLen }).map((x, idx) => [tokenAddress, Number(idx)]);
    const stateOfPositions = await Promise.all(checkPositions.map(args => this.stayKingContract.isKillable(...args)));

    return stateOfPositions.map<KillablePosition>((killable: boolean, positionId) => ({positionId, killable})).filter(item => item.killable);
  }
